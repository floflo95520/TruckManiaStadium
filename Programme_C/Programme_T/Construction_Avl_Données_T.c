...
Avl* construireAvl(Avl * a, int taille){
    ...
    r=fscanf(fichier, "%d %d %s %s", &tempRouteID, &tempNumEtape, ville_a, ville_b); // On récupère la première ligne du fichier data_t.txt
    if(r!=4){
        printf("Erreur lors de la récupération des données. Fin du programme.");
        exit(5);
    }
    a = creerAvl(ville_a, tempRouteID, tempNumEtape); // On s'en sert pour créer le premier noeud de l'avl
    ...
    a = insertionAvl(a, &equilibre, tempRouteID, 0, ville_b);


    for(int i = 1; i<taille;i++){
        r=fscanf(fichier, "%d %d %s %s", &tempRouteID, &tempNumEtape, ville_a, ville_b);
        if(r!=4){
        printf("Erreur lors de la récupération des données. Fin du programme.");
        exit(5);
    }
    // on insère chaque ville qu'on récupère tout en gardant le 0 pour la ville d'arrivée de l'étape
        a = insertionAvl(a, &equilibre, tempRouteID, tempNumEtape, ville_a);
        a = insertionAvl(a, &equilibre, tempRouteID, 0, ville_b); 
