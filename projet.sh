...
/usr/bin/time -f "%e" awk -F';' '!seen[$1,$6]++ { count[$6]++ } END {for (name in count) print count[name],";" name}' $1 | sort -r -n -k1,1 | head -n10 > /dev/null 
gnuplot << EOF
reset
...
/usr/bin/time -f "%e" ./Programme_C/ProgS $t
gnuplot << EOF
reset
